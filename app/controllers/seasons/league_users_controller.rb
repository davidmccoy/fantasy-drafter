class Seasons::LeagueUsersController < ApplicationController
  ...
  def create
    if request.referrer.include? "join"
      ...
      if league_user.save
        ...
        redirect_to game_season_league_join_path(@league.leagueable.game,@league.leagueable, @league) and return
      end
    else
  ...
  def update
    if @league_user.update(confirmed: true)
      ...
      redirect_to game_season_league_path(@league.leagueable.game,@league.leagueable, @league)
    else
      ...
      redirect_to game_season_league_league_user_confirm_url(@league.leagueable.game,@league.leagueable, @league, @league_user)
    end
  end
    ...
    if league_user
      if league_user.user == @league.admin
        ...
        if league_user.destroy
          ...
          redirect_to game_season_league_league_user_confirm_url(@league.leagueable.game,@league.leagueable, @league, league_user) and return
        end
  ...
  def confirm
    ...
    redirect_to game_season_league_path(@game, @season, @league) unless @league_user
  end
  ...
  def resend_invite
    ...
    redirect_to game_season_league_league_users_path(@league_user.league.leagueable.game, @league_user.league.leagueable, @league_user.league)
  end
